...
| 11 | [Integration readiness (IR)](integration-readiness/IR_CONTRACT_AND_SLICE_PLAN.md) | Scope drafted, pending review; scheduled after R3 and before Step 3.5 so the codebase's integration surface is corrected before more analyzers are added on top of it. **This row, like the linked contract file, is a stale snapshot inherited from `main` on `fix/ir4-python-version-reproducibility` — `feat/ir-integration-readiness` has since landed most of IR (§2 item 7/§7 of that branch's own copy of the linked contract). IR.4 (Python-version reproducibility), implemented on this fix branch, is documented in that same contract file's ported §6, added here 2026-09-26 per explicit project-owner direction for single-diff PR review — see that section's own note on why, and on the documentation-merge-conflict tradeoff accepted to get it.** |
